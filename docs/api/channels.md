...
| characters（全体） | 全キャラクターのボイスプロバイダーが同一であること |
...
| 追加するキャラクターのボイスプロバイダーが既存キャラクターと同一であること |

**レスポンス（201 Created）:** `ChannelDataResponse`

**エラー:**
- `400` — キャラクターが既に2人登録済み / 既にチャンネルに追加済み / ボイスプロバイダーが不一致
...
| 置換先のキャラクターのボイスプロバイダーが残りのキャラクターと同一であること |

**レスポンス（200 OK）:** `ChannelDataResponse`

**エラー:**
- `400` — 置換先キャラクターが既にチャンネルに追加済み / ボイスプロバイダーが不一致
- `403` — オーナーでない / キャラクターの所有権がない
