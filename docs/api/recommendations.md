...
| ログイン | ユーザーの再生履歴・再生リストに基づくパーソナライズ |
...
2. **パーソナライズスコア**:
   - 再生履歴のカテゴリ傾向に一致 → 加点
   - デフォルト再生リスト（後で聴く）に追加したエピソードのチャンネルカテゴリ → 加点
...
2. **優先度の高い順に混合**:
   ...
   - **デフォルト再生リスト（後で聴く）**: 追加済みで未再生のエピソード（最大 3 件）
...
| inDefaultPlaylist | bool | デフォルト再生リスト（後で聴く）に追加済みか（ログイン時のみ） |
