...
│    - channel.scriptPrompt（チャンネル全体方針）                              │
│    - channel.characters（登場人物情報）                                     │
│    - prompt（ユーザー入力）                                                 │
│ 6. LLM API 呼び出し（台本生成）                                             │
│ 7. LLM レスポンスをパース（ScriptLine 配列に変換）                          │
│ 8. episode.scriptPrompt に prompt を保存                                    │
│ 9. 台本行を DB に保存                                                       │
...
| scriptPrompt | channel.scriptPrompt | チャンネル全体の方針（「明るく楽しい雰囲気で」など） |
| characters | channel.characters | 登場人物リスト（名前、ペルソナ） |
...
## 登場人物
- 太郎: 明るく元気な性格。語尾に「だよね」をつけがち。
- 花子: 落ち着いた知的な性格。論理的に話す。
