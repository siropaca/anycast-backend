...
- 略語・英字略称は正式名称で言い換える（×「PN」→ ○「ペンネーム」）。TTS が正しく読めない略語をそのまま使わない
...
func getPhase3SystemPrompt(talkMode script.TalkMode, withEmotion bool, durationMinutes, episodeNumber int) string {
	...
	sb.WriteString("- 略語・英字略称は初出時に正式名称で言い換える（×「PN」→ ○「ペンネーム」、×「OSSに貢献」→ ○「オープンソースソフトウェアに貢献」）。TTS が正しく読めない略語をそのまま使わない\n")
