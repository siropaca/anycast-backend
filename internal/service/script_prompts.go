...
## 過去エピソードとの差別化
- ブリーフに previous_episodes が含まれる場合、過去に扱ったテーマと切り口の重複を避ける
- 過去エピソードで取り上げた具体例・数字・事例とは異なるものを選ぶ
- 過去の話題を前提知識として活用し、より発展的な内容を提供する

## 制約
- 素材にコード表現（関数名・変数名・構文）を含めない。音声で伝わる日本語表現に置き換える（×「while(true)」→ ○「無限ループ」）
- 略語・英字略称は正式名称で記載する（×「PN」→ ○「ペンネーム」）。ただし TTS が正しく読める一般的な製品名・技術用語（例: Node.js, Python, GitHub, API 等）はそのままでよい
- 素材は各カテゴリ最低3個ずつ用意する
...
// getPhase4SystemPrompt は Phase 4（リライト）用のシステムプロンプトを返す
//
// withEmotion が true の場合は感情タグの追加指示を含め、false の場合は感情タグを付けない指示にする
func getPhase4SystemPrompt(withEmotion bool) string {
	var sb strings.Builder

	sb.WriteString(`あなたはポッドキャスト台本のリライト担当です。
...
- 自分の名前で自分を呼ぶ三人称表現（例:「あかりもそう思ってました」）があれば、「私」か「僕」に書き換える
...
- コード・数式の表現は音声で伝わる日本語に置き換える（×「while(true)」→ ○「無限ループ」、×「setTimeout」→ ○「タイマー処理」）。変数名・関数名・構文をそのまま台詞に入れない
- 略語・英字略称は正式名称で言い換える（×「PN」→ ○「ペンネーム」）。ただし TTS が正しく読める一般的な製品名・技術用語（例: Node.js, Python, GitHub, API 等）はそのままでよい
...
// getPhase5SystemPrompt は Phase 5（QA パッチ修正）用のシステムプロンプトを返す
//
// withEmotion が true の場合は感情タグの修正指示を含め、false の場合は感情タグを付けない指示にする
func getPhase5SystemPrompt(withEmotion bool) string {
	var sb strings.Builder

	sb.WriteString(`あなたはポッドキャスト台本の品質管理担当です。
...
func getPhase3SystemPrompt(talkMode script.TalkMode, withEmotion bool, durationMinutes, episodeNumber int) string {
	...
	sb.WriteString("- コード・数式の表現は音声で伝わる日本語に置き換える（×「while(true)で回す」→ ○「無限ループで回す」、×「setTimeout」→ ○「タイマー処理」）。変数名・関数名・構文をそのまま台詞に入れない\n")
	sb.WriteString("- 略語・英字略称は初出時に正式名称で言い換える（×「PN」→ ○「ペンネーム」、×「OSSに貢献」→ ○「オープンソースソフトウェアに貢献」）。ただし TTS が正しく読める一般的な製品名・技術用語（例: Node.js, Python, GitHub, API 等）はそのままでよい\n")
	...
	sb.WriteString("- 一人称は「私」（男女問わない）か「僕」（キャラクターに合わせる）を使う。自分の名前で自分を呼ぶ三人称表現は禁止（×「あかりもそう思う」→ ○「私もそう思う」）\n")
	if talkMode == script.TalkModeDialogue {
