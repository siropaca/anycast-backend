...
func (m *mockEpisodeRepositoryForChannel) Search(ctx context.Context, filter repository.SearchEpisodeFilter) ([]model.Episode, int64, error) {
...
func (m *mockEpisodeRepositoryForChannel) CountPublishedByChannelIDs(ctx context.Context, channelIDs []uuid.UUID) (map[uuid.UUID]int, error) {
	args := m.Called(ctx, channelIDs)
	return args.Get(0).(map[uuid.UUID]int), args.Error(1)
}

func (m *mockEpisodeRepositoryForChannel) CountByChannelIDBeforeCreatedAt(ctx context.Context, channelID uuid.UUID, createdAt time.Time) (int64, error) {
