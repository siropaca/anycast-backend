...
func (s *episodeService) toEpisodeResponse(ctx context.Context, e *model.Episode, owner *model.User, playback *model.PlaybackHistory) (response.EpisodeResponse, error) {
	...
	if e.VoiceAudio != nil {
		signedURL, err := s.storageClient.GenerateSignedURL(ctx, e.VoiceAudio.Path, storage.SignedURLExpirationAudio)
		if err != nil {
			return response.EpisodeResponse{}, err
		}
		resp.VoiceAudio = &response.AudioResponse{
			ID:         e.VoiceAudio.ID,
			URL:        signedURL,
			MimeType:   e.VoiceAudio.MimeType,
			FileSize:   e.VoiceAudio.FileSize,
			DurationMs: e.VoiceAudio.DurationMs,
		}
	}
